...
var EditAddressListStore = require('../stores/EditAddressListStore.js');
var EditAddressListUtil = require('../utils/EditAddressListUtil.js');
var EditAddressListActionCreators = require('../actions/EditAddressListActionCreators.js');
function getStateFromStores() {
	return {
		editAddress: EditAddressListStore.getData(),
		err: EditAddressListStore.getErr()
	};
}
...
var SelectAddress = React.createClass({
	...
	changeAddress: function(dataOptions, e) {
		...
		switch (e.target.name) {
			case "country":
				EditAddressListUtil.getAddress(dataOptions.child, id, e.target.name, e.target.value);
				break;
			case "state":
				EditAddressListUtil.getAddress(dataOptions.child, id, e.target.name, e.target.value);
				break;
			case "city":
				...
				break;
			case "region":
				EditAddressListUtil.getAddress(null, null, e.target.name, e.target.value);
				break;
		}
	},
	render: function() {
...
var EditAddress = React.createClass({
	...
	componentDidMount: function() {
		EditAddressListUtil.getData();
		EditAddressListStore.addChangeListener(this._onChange);
	},

	componentWillUnmount: function() {
		EditAddressListStore.removeChangeListener(this._onChange);
	},
	render: function() {
		return (
			<div className="row">
	...
	_handleSubmit: function(e) {
		...
		if (FormUtil.isSafari()) {
			if (!e.target.checkValidity()) {
				...
				if (!data.name) {
					EditAddressListActionCreators.submitEditAddress(ErrMsgMap.receiverRequired);
				} else if (!data.phone) {
					EditAddressListActionCreators.submitEditAddress(ErrMsgMap.phoneRequired);
				} else if (!(data.phone).match(phonePattern)) {
					EditAddressListActionCreators.submitEditAddress(ErrMsgMap.phonePattern);
				} else if (!data.country) {
					EditAddressListActionCreators.submitEditAddress(ErrMsgMap.areaRequired);
				} else if (!data.zipcode) {
					EditAddressListActionCreators.submitEditAddress(ErrMsgMap.zipCodeRequired);
				} else if (!data.addrDetail) {
					EditAddressListActionCreators.submitEditAddress(ErrMsgMap.detailRequired);
				}

				return false;
			}
			else
				EditAddressListActionCreators.submitEditAddress('');
		}
		data.deliveraddrId = JSON.parse(sessionStorage.getItem('deliveraddrId')).deliveraddrId;
		EditAddressListUtil.submitAddress(data);
		this.history.pushState(null, "/addresslist");
		return false;
	},
	_onChange: function() {
